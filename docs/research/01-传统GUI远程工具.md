...
- **面向开发者的新功能（2026）**：
  - **终端**：2026-05 前后上线「零配置终端命令行」，被控设备在线即可直接进入其命令行，无需 SSH 配置或内网穿透。**支持会话分离与重新接入**：关掉窗口远端命令继续跑，回来提示「接入已有会话」；但被控端客户端重启或命令行进程退出后会话即丢失。
  - **端口映射**：2026-04/05 上线，选一台在线设备作跳板，把远端 MySQL/Redis/NAS/Web 服务的 TCP 端口映射到本地，纯图形化操作，官方口径是「复用已有的远控连接通道」。
  - **CLI**：2026-06 前后上线 `uuyc-cli`（安装目录 `bin` 下），覆盖「本地设备、云电脑、远程 Shell、远程协助」，官方明说可「嵌入批处理、Shell、AI 流程」；但依赖主客户端运行，关掉主程序返回退出码 2。
- **传输架构**：P2P + 网易中转；不可自建。
...
- **意义**：证明「远控厂商正在把终端、端口映射当成开发者卖点」。**其终端已经不是「单机单终端」**——有会话分离与重新接入、有 CLI、有端口映射；缺的是 AI 事件通知（谁在等我确认）、多机 × 多会话聚合视图、Web 页面入口与可自建。详见 [调研 07 · UU远程终端实现分析](07-UU远程终端实现分析.md)。
...
| 网易 UU远程 | 网易免费远控 | 全功能免费（4K144） | 无 | P2P + 网易中转 | 否 | 2026 零配置终端（含会话分离/重接）+ 端口映射 + uuyc-cli | 支持，键盘体验待实测 | 好 | 稳定性/掉线投诉；免费可持续性存疑 |
...
- **远控厂商补终端**：RustDesk 1.4.1 Terminal（2025-08）→ 1.4.5 移动端终端浮动键；网易 UU远程 2026 年上线零配置终端与端口映射；向日葵 CMD/SSH 仍锁在精英版；ToDesk 手机端无 CMD。**其中 UU远程 已经走得最远**：终端支持会话分离/重新接入，另有 CLI 与端口映射，不再只是「单机 shell」；但仍是哑终端，没有 AI 事件感知与多机聚合（见 [调研 07](07-UU远程终端实现分析.md)）。
