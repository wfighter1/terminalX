...
**你自己部署的 AI 代理监工台。** Windows 上的 Claude Code / Codex / Grok Build 会话常驻不掉线，手机上一屏看清谁在等你、一键处理，任何 API 配置都能远程。

> 状态：产品设计与原型阶段（2026-09）。尚无可运行代码。本仓库目前是调研、设计文档与可交互原型。
...
- **ToDesk / 向日葵 / RustDesk** 传的是像素：手机上终端字看不清、虚拟键盘没有 Esc / Ctrl、没有「谁在等我」、免费额度反复缩水、公司禁装。
...
- Claude Code：http hooks 两种审批模式（默认通知，可选远程优先）；Codex：hooks.json 登记 + 打开终端；Grok Build：尽力；MiniMax：供应商预设
